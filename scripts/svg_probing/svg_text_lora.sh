...
# # natural image: vsr task
# TASK_NAME="vsr"
# echo "run task: ${TASK_NAME}"
# DATA_PATH=${DATA_DIR}/svg_probing/vsr/train_sam_topk-30_svg_postprocessed_simp-10_truncated_2300.json
# OUTPUT_DIR=${CODE_DIR}/checkpoints/svg_probing/svg_text_lora/${TASK_NAME}
# LR=2e-5
# NUM_EPOCH=3
...
#############################################
# natural image: whats_up task

TASK_NAME="whats_up"
echo "run task: ${TASK_NAME}"
DATA_PATH=${DATA_DIR}/svg_probing/whats_up/train_svg_truncated_2300.json
OUTPUT_DIR=${CODE_DIR}/checkpoints/svg_probing/svg_text_lora/${TASK_NAME}
LR=2e-5
NUM_EPOCH=3
deepspeed --include localhost:5,6 llava/train/train_mem.py \
    --lora_enable True --lora_r 128 --lora_alpha 256 \
    --deepspeed ${CODE_DIR}/scripts/${DEEPSPEED}.json \
    --model_name_or_path ${MODEL_PATH} \
    --version v1 \
